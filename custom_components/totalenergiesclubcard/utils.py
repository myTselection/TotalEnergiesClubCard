...
class ComponentSession(object):
    ...
    def login(self, cardnumber, password):
        response = self.s.get("https://services.totalenergies.be/nl/inloggen-op-uw-club-account",timeout=30,allow_redirects=True, verify=False)
        ...
        response = self.s.post("https://club.totalenergies.be/authentification/authentification.php?PAYS=BE&LG=NL",data=data,timeout=30,allow_redirects=False, verify=False)
    ...
    def transactions(self):
        response = self.s.get("https://club.totalenergies.be/adherent_transactions/transactions.php?PAYS=BE&LG=NL",timeout=30,allow_redirects=True, verify=False)
        ...
        for row in rows:
            ...
            if len(columns) >= 4:
                transaction = {
                    ...
                    'station': columns[0].contents[2] if len(columns[0].contents) >= 3 else 'Unknown',
